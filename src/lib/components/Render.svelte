<script lang="ts">
  ...
  //     const text = `Página ${pageNumber} processada!`;
  ...
  const applyStatusCanvasStyles = (
    ...
    if (processedPages.includes(pageNumber)) {
      ...
      bgColor = 'rgba(186, 79, 125, 0.2)'; // Slightly more opaque
      textColor = 'rgba(156, 49, 95, 1)'; // Darker for better contrast
      borderColor = 'rgba(186, 79, 125, 1)'; // Full opacity for stronger border
    } else if (processingPages.includes(pageNumber)) {
      text = `Página ${pageNumber} em processamento...`;
      bgColor = 'rgba(255, 223, 186, 0.5)'; // Light orange for processing
      textColor = 'rgba(186, 79, 25, 1)'; // Dark orange for contrast
      borderColor = 'rgba(255, 165, 0, 0.8)'; // Orange border
    } else if (selectedPages.includes(pageNumber)) {
      text = `Página ${pageNumber} selecionada`;
      bgColor = 'rgba(173, 216, 230, 0.5)'; // Light blue for selection
      textColor = 'rgba(0, 90, 156, 1)'; // Dark blue for contrast
      borderColor = 'rgba(70, 130, 180, 0.8)'; // Steel blue border
    } else {
      return;
    }
    ...
    // Adjust the text outline for better contrast
    ...
    >
      {#each selectedPages as page}
        <Button
          ...
          size="sm"
          on:click={() => handlePageNumberClick(page)}
        >
          {page}
        </Button>
      {/each}
